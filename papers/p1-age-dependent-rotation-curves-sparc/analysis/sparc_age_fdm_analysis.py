...
OUTPUTS:
  ...
  SPARC_age_fdm_partial_correlations.csv   (partial r via residual method)
  SPARC_age_fdm_regression_models.csv      (OLS; age coefficient across models)
  SPARC_age_fdm_binned_bootstrap.csv       (qcut bins in log10(Vmax); bootstrap CI for old-young Δf_DM)
...
IN_PATH = "sparc_age_fdm_data.csv"
OUT_DIR = "."
...
def main():
    ...
    corr.to_csv(os.path.join(OUT_DIR, "SPARC_age_fdm_correlations.csv"), index=False)
    ...
    pd.DataFrame(partial_rows).to_csv(os.path.join(OUT_DIR, "SPARC_age_fdm_partial_correlations.csv"), index=False)
    ...
    for Xcols in [[],["logVmax"],["logRmax"],["logVmax","logRmax"],["logVmax","logRmax","logSB0"]]:
        ...
        reg_rows.append({
            ...
            "n":n,"beta_age":beta_age,"se_age":se_age,"t_age":t_age,"p_age":p_age
        })
    pd.DataFrame(reg_rows).to_csv(os.path.join(OUT_DIR, "SPARC_age_fdm_regression_models.csv"), index=False)
    ...
    pd.DataFrame(rows).to_csv(os.path.join(OUT_DIR, "SPARC_age_fdm_binned_bootstrap.csv"), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "SPARC_age_vs_fdm_scatter.png"), dpi=200)
