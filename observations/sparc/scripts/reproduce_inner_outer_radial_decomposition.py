...
def subset_masks(df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    Disk: bulge-free / disk-dominated objects, approximated by bulge_frac_outer == 0.
    Disk+Q=1: both conditions.
    """

    q_is_one = pd.to_numeric(df["Q"], errors="coerce") == 1
    disk = pd.to_numeric(df["bulge_frac_outer"], errors="coerce").fillna(np.inf) == 0
    return {
